...
public class NicoMessage {
	...
	public HashMap<String, NicommunityData> getNicommunityData(Document document){
		HashMap<String, NicommunityData> nicommunityData = new HashMap<String, NicommunityData>();
		for (int i=0; i < getNodeList(document,"community_id").getLength();i++){
			nicommunityData.put(getNodeValue(document,"community_id", i),
					new NicommunityData(getNodeValue(document,"community_id", i),"",""));
		}
		return nicommunityData;
	}
	public void setCommunityInfo(String document, NicommunityData nicommunityData){
		Matcher matcher = _communityNamePattern.matcher(document);
		if(matcher.matches()){
			nicommunityData.CommunityName = matcher.group(1);
		}
		matcher = _communityOwnerPattern.matcher(document);
		if(matcher.matches()){
			nicommunityData.CommunityOwner = matcher.group(1);
		}
	}
	public void setChannelInfo(String document, NicommunityData nicommunityData){
		Matcher matcher = _channnelNamePattern.matcher(document);
		if(matcher.matches()){
			nicommunityData.CommunityName = matcher.group(1);
		}
		nicommunityData.CommunityOwner = "";
	}
